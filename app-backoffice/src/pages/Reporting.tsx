...
import { dailyCA, caByCategory, passengerData } from '../data/mock';
...
const G = ['#3D75C4','#2EAD78','#B8914A','#C47B35','#7A5FC4','#2E9FAD','#C9524A'];
...
export default function Reporting() {
  ...
  const total30 = dailyCA.reduce((s,d)=>s+d.ca,0);
                ...
                <tr key={c.category}>
                  <td>
                    <div style={{display:'flex',alignItems:'center',gap:6}}>
                      ...
                      <span style={{color:'var(--text)',fontWeight:i===0?600:400}}>{c.category}</span>
                    </div>
                  </td>
                  ...
                  <td>
                    <div style={{display:'flex',alignItems:'center',gap:7}}>
                      <div style={{flex:1,height:3,background:'var(--surface-3)',borderRadius:2}}>
                        <div style={{height:'100%',width:`${c.part}%`,background:G[i%G.length],borderRadius:2,transition:'width 0.5s'}}/>
                      </div>
                      <span style={{fontFamily:'IBM Plex Mono,monospace',fontSize:10,color:'var(--text-3)',minWidth:26}}>{c.part}%</span>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="card" style={{padding:'18px 20px'}}>
            ...
            <BarChart data={passengerData} barSize={16} margin={{top:2,right:2,bottom:0,left:-20}}>
              ...
              <Bar dataKey="tickets" radius={[2,2,0,0]}>
                {passengerData.map((_,i)=>(
                  <Cell key={i} fill={i===passengerData.length-1?'#B8914A':'var(--surface-4)'}/>
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginTop:10}}>
            {passengerData.slice(-3).map(p=>(
              <div key={p.month} style={{background:'var(--surface-2)',border:'1px solid var(--border)',borderRadius:6,padding:'9px 11px',textAlign:'center'}}>
                <div className="section-label" style={{marginBottom:3}}>{p.month}</div>
