...
export default function POS() {
  ...
  const categories = useMemo(() => {
    ...
    products.forEach(p => {
      if (p.categorie) uniqueCategories.add(p.categorie);
    });
  ...
  // Forcer le re-rendu quand le panier change
  const [, forceUpdate] = useState({});
  useEffect(() => {
    forceUpdate({});
  }, [cart, cartTotal, activeCurrency]);
                ...
                return (
                  <button key={p.id} onClick={() => !out && addToCart(p.id)} disabled={out}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 7 }}>
                      <span style={{ fontSize: 8.5, fontWeight: 700, color: 'var(--text-3)', background: 'var(--surface-3)', padding: '1px 5px', borderRadius: 3, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{String(p.categorie || '').substring(0, 5)}</span>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 13 }}>
                <div>
                  ...
                  <div style={{ fontFamily: 'IBM Plex Mono,monospace', fontSize: 28, fontWeight: 700, color: 'var(--accent)', lineHeight: 1 }}>
                    {console.log('DEBUG POS - activeCurrency:', activeCurrency, 'total:', total, 'fmt result:', fmt(total, activeCurrency))}
